...
class StudioBrowser:
    ...
    def _wait_until_topmost(self, element, timeout=2.0):
    ...
    def playback_click(self, payload: dict) -> PlaybackStepResult:
        ...
        def do_click(element):
            self._logger.info("-------- do_click(element) --------")
            self._logger.info(f"Element: {element}")
            self._logger.info("-------- /do_click(element) -------")

            self._wait_for_dom_stable(timeout=1.0, stable_time=0.2)

            self._wait_until_topmost(element)
    ...
    def _playback_element(self,
        ...
        return PlaybackStepResult.fail("Playback failed unexpectedly")
